...
class IntentSceneDataset(Dataset):
    def __init__(self, dataset_directory: str = r"E:\famous_datasets\nirscene1", transform=transforms.Compose([transforms.ToTensor(), lambda x: 2.0 * x - 1]), train_data_percent:float = .80, test_data:bool = False):
        ...
        for root, dirs, files in os.walk(dataset_directory):
            ...
            if test_data: files = files[start_point:] #Spliting test and train data
            else: files = files[:start_point]
            for name in files:
                if self.default_data_parser(name): rgb_data.append(os.path.join(root, name))
                else: nir_data.append(os.path.join(root, name))
    ...
    def get_length(length:int, percent_of_train:float):
        ...
        if start_point % 2 == 0:
            return start_point
        else: return start_point -1

    def __len__(self):
        assert len(self.label_list) == len(self.rgb_data) and len(self.nir_data) == len(self.rgb_data), "The data has miss match lengths"
